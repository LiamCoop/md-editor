...
export function useComments({
    ...
    // Remove comments whose anchor range is no longer valid after text deletion
    useEffect(() => {
        if (!activeDoc?.comments?.length || !activeDoc.content) {
            return;
        }
        const contentLength = activeDoc.content.length;
        const invalidCommentIds = activeDoc.comments
            .filter((comment) => comment.anchorStart >= contentLength || comment.anchorEnd > contentLength)
        ...
        changeActiveDoc((doc) => {
            ...
            for (let i = doc.comments.length - 1; i >= 0; i--) {
                if (invalidCommentIds.includes(doc.comments[i].id)) {
                    doc.comments.splice(i, 1);
                }
            }
        });

        // Clean up UI state for removed comments
        for (const id of invalidCommentIds) {
            if (hoveredCommentId === id) {
                setHoveredCommentId(null);
            }
            ...
            if (openCommentMenuId === id) {
                setOpenCommentMenuId(null);
            }
        }
    }, [activeDoc, changeActiveDoc, hoveredCommentId, replyingToCommentId, editingCommentId, openCommentMenuId]);
